...
class PDB():
	SAMPLE_RATE  = 10 # number of frames to skip (sparser sampling)
	MAX_VALUE    =  999999999999999 # arbitrarily large number
	MIN_VALUE    = -999999999999999 # arbitrarily small number
	ANGSTROM_PRECISION  = 624 # unit scaling based on testing
...
class PeptideCSV():
	...
	def _get_hilbert(self):
		...
		with open(PDB.PATHS[0], 'r') as pdb:
			...
			for line in pdb:
				...
				if id == "MODEL":
					...
					if frame % PDB.SAMPLE_RATE == 0:
						...
						self.data[frame // PDB.SAMPLE_RATE - 1].append([x, y, z])
